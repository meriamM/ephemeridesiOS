...
$rs1 = $mysqli->query("SELECT idTheme FROM ChoixTheme where idClient=$idclient");

while ($themes=mysqli_fetch_assoc($rs1))
{  $t=$themes["idTheme"] ;

$rs = $mysqli->query("SELECT * FROM ephemerides where idTheme=$t and jour>=$jour and mois=$mois");
